...
import { withDefaults, ref, computed, unref, watch, nextTick, onMounted, Ref, onBeforeUnmount } from 'vue'
...
const selectLabel = computed(()=> {
	if ( isShowList.value || isShowList.value ) {
		return tempSelectItem.value.label
	}
	return (
		(showList.value.find(item => item.value === selectValue.value) || {}).label || selectValue.value
	)
})

// 初始展示的数据列表
const initScroll = () => {
	const index = props.options.findIndex(item=> item.value === selectValue.value)
...
const initWrap = () => {
	// 滚动容器的高度
	unref(listWrap).style['max-height'] = props.lineHeight * minValue(props.showNum, maxValue(props.options.length, 1)) + 'px'
	// 计算总的数据高度， 构建滚动条高度
	unref(scrollbar).style.height = props.lineHeight * maxValue(props.options.length, 1) + 'px'
}
...
const initValue = () => {
	...
	// 初始化回显值的依赖数据
	const temp = props.options.find(item => item.value === selectValue.value) || { label: selectValue.value, value: selectValue.value }
